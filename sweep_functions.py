...
class three_field_joint_sweep_cartesian(Soft_Sweep):
    """
    Allows jointly sweeping two parameters while preserving their
    respective ratios.
    Allows par_A and par_B to be arrays of parameters
    """
    def __init__(self, x_field, y_field, z_field, field_direction, **kw):
        self.set_kw()
        self.name = 'total field'
        self.parameter_name = 'total field'
        self.sweep_control = 'soft'

        self.unit = x_field.unit
    ...
    def set_parameter(self, val):
        self.x_field.set(val*self.field_direction[0]/(np.linalg.norm(self.field_direction)))
        self.y_field.set(val*self.field_direction[1]/(np.linalg.norm(self.field_direction)))
        self.z_field.set(val*self.field_direction[2]/(np.linalg.norm(self.field_direction)))
        
        #if self.retrieve_value:
        #    if self.instr:
        #        self.instr.operationComplete()  # only get first one to prevent overhead
        
class three_field_joint_sweep_spherical(Soft_Sweep):
    """
    Allows jointly sweeping two parameters while preserving their
    respective ratios.
    Allows par_A and par_B to be arrays of parameters
    """
    def __init__(self, x_field, y_field, z_field, fixed_angle, total_field, rotation_angle_index, **kw):
        self.set_kw()
        self.name = rotation_angle_index
        self.parameter_name = rotation_angle_index
        self.sweep_control = 'soft'
        ...
        self.rotation_angle_index = rotation_angle_index
       # self.retrieve_value = retrieve_value
       # self.instr=instr
       # if preserve_ratio:
       #     try:
    ...
    def set_parameter(self, val):
        ...
        if self.rotation_angle_index == 'theta':
            
            self.x_field.set(self.total_field*np.sin(val)*np.cos(self.fixed_angle))
            self.y_field.set(self.total_field*np.sin(val)*np.sin(self.fixed_angle))
            self.z_field.set(self.total_field*np.cos(val))
            
            
        elif self.rotation_angle_index == 'phi':
            
            self.x_field.set(self.total_field*np.sin(self.fixed_angle)*np.cos(val))
            self.y_field.set(self.total_field*np.sin(self.fixed_angle)*np.sin(val))
            self.z_field.set(self.total_field*np.cos(self.fixed_angle))
            
        else:
            raise ValueError("undifined rotation angle")
...
class KST_VNA_sweep(Hard_Sweep):

    def __init__(self, VNA,
                 start_freq=None, stop_freq=None,
                 center_freq=None, span=None, power=None, delay_t = 5.38e-8,
                 segment_list=None, measure = 'S21',
                 npts=100, if_bandwidth = None, force_reset=False,
                 sweep_delay = 1e-4):
        '''
        Frequencies are in Hz.
        Defines the frequency sweep using one of the following methods:
        ...
        If force_reset = True the VNA is reset to default settings
        '''
        super(KST_VNA_sweep, self).__init__()
        self.name = 'KST_VNA_sweep'
        self.parameter_name = 'frequency'
        self.unit = 'Hz'
    ...
    def prepare(self):
        ...
        Set the frequency sweep and get the frequency points back from the insturment
        '''
        str_to_write = "CALC1:PAR:SEL 'CH1_S11_1',fast"
        self.VNA.visa_handle.write(str_to_write)
        str_to_write = "CALC1:PAR:MOD:EXT '%s'" % self.measure
        ...
        self.VNA.min_sweep_time('on')
        # start a measurement once the trigger signal arrives
        self.VNA.trigger_source('MAN')
        #setup the VNA power.
        self.VNA.power(self.power)
        # trigger signal is generated with the command:
        # VNA.start_sweep_all()
        # setup which parameter to measure
        
         
        self.VNA.bandwidth(self.if_bandwidth)
        self.VNA.delay_t(self.delay_t)
        
#         print(self.VNA.delay_t(),"delay time") # for debugging purposes

        self.VNA.rf_on()
        
        if self.segment_list == None:
            self.VNA.sweep_type('LIN')  # set a linear sweep
            if self.start_freq != None and self.stop_freq != None:
                self.VNA.start_frequency(self.start_freq)
                self.VNA.stop_frequency(self.stop_freq)
            elif self.center_freq != None and self.span != None:
            ...
            self.sweep_points = self.VNA.get_stimulus()
            self.VNA.init_sweep_dwell_time(self.sweep_delay)
            ...
            # str_to_write = 'SENS:SEGM:LIST SSTOP 2, %s, %s, %s, %s, %s, %s, %s, %s' % (str(self.segment_list[0]), str(self.segment_list[1]), \
            #     str(self.segment_list[2]), str(self.segment_list[3]), str(self.segment_list[4]), \
            #     str(self.segment_list[5]), str(self.segment_list[6]), str(self.segment_list[7]))
            

            self.VNA.segment_power_control()

            self.VNA.add_first_segment()
            self.VNA.first_segment_turn_on()
            self.VNA.start_freq_first_segment(self.segment_list[0])
            self.VNA.stop_freq_first_segment(self.segment_list[1])
            self.VNA.npts_first_segment(self.segment_list[2])
            self.VNA.bandwidth_first_segment(self.segment_list[3])
            self.VNA.power_first_segment(self.segment_list[4])
            
            
            self.VNA.add_second_segment()
            self.VNA.second_segment_turn_on()
            self.VNA.start_freq_second_segment(self.segment_list[5])
            self.VNA.stop_freq_second_segment(self.segment_list[6])
            self.VNA.npts_second_segment(self.segment_list[7])
            self.VNA.bandwidth_second_segment(self.segment_list[8])
            self.VNA.power_second_segment(self.segment_list[9])
            
            self.VNA.sweep_type('SEGM')  # set a segment sweep
            
            
            # self.VNA.sweep_type('SEGM')  # set a segment sweep
            
            # # self.VNA.add_segment()
            # # self.VNA.segment_on('on')
            
            # str_to_write = 'SENS:SEGM:LIST SSTOP 2, 1, 101, 3E9, 4E9, 1, 151, 6.39E9, 6.41E9'
            
            # self.VNA.write_raw(str_to_write)
            
            # self.VNA.start_frequency_segment(self.segment_list[0])
            # self.VNA.stop_frequency_segment(self.segment_list[1])
            # self.VNA.npts_segment(self.segment_list[2])
            
            # self.VNA.npts_segment(302)
            
            self.sweep_points = self.VNA.get_stimulus_segment(self.segment_list)
...
class KST_VNA_sweep_end_trigger_multi_QDac_list(Hard_Sweep):

    def __init__(self, VNA,
                 start_freq=None, stop_freq=None,
                 center_freq=None, span=None, power=None, delay_t = 5.38e-8,
                 segment_list=None, measure = 'S21',
                 npts=100, if_bandwidth = None, 
                 qdac_channel = None,qdac_dc_start = None,qdac_dc_stop = None,
                 qdac_dc_npts = None,
                 force_reset=False):
        '''
        Frequencies are in Hz.
        ...
        If force_reset = True the VNA is reset to default settings
        '''
        super(KST_VNA_sweep_end_trigger_multi_QDac_list, self).__init__()
        self.name = 'KST_VNA_sweep'
        self.parameter_name = 'frequency'
        self.unit = 'Hz'
        ...
        self.qdac_dc_start = qdac_dc_start
        self.qdac_dc_stop = qdac_dc_stop
        self.qdac_dc_npts = qdac_dc_npts
    ...
    def prepare(self):
        ...
        # setup the VNA trigger output:
            
        
        
        # turn on VNA RF output
        ...
        if self.segment_list == None:
            ...
            self.VNA.trigger_output('on')
            self.VNA.trigger_output_duration(2e-5)
            self.VNA.trigger_output_mode('SWE')
            self.VNA.trigger_output_sign('POS')
            self.VNA.trigger_output_pos('AFT')
            self.VNA.sweep_dwell_time(1e-4)#set vna dwell time for debug 
        ...
        # define the QDac multichannels to be triggered at the begining of each VNA sweep
        
        for channel in self.qdac_channel:
            channel.output_filter('med')
            time.sleep(0.1)
            dc_list = channel.dc_list(voltages=np.linspace(self.qdac_dc_start[self.qdac_channel.index(channel)],
                                                           self.qdac_dc_stop[self.qdac_channel.index(channel)],
                                                           self.qdac_dc_npts).tolist(),
                                                           dwell_s = 1e-4,
                                                           stepped = True)
            ''' Set the channel trigger input to external'''
            time.sleep(3)
            dc_list.start_on_external(1)
...
class KST_VNA_sweep_end_trigger_multi_QDac_list_custom(Hard_Sweep):
    ...
    def __init__(self, VNA,
                 ...
                 segment_list=None, measure = 'S21',
                 npts=100, if_bandwidth = None, 
                 qdac_channels = None, qdac_dc_lists = None,
                 force_reset=False):
        '''
        Frequencies are in Hz.
        Defines the frequency sweep using one of the following methods:
        ...
        If force_reset = True the VNA is reset to default settings
        '''
        super(KST_VNA_sweep_end_trigger_multi_QDac_list_custom, self).__init__()
        self.name = 'KST_VNA_sweep'
        self.parameter_name = 'frequency'
        self.unit = 'Hz'
        ...
        self.qdac_dc_lists = qdac_dc_lists
    ...
    def prepare(self):
        ...
        for channel in self.qdac_channels:
            channel.output_filter('med')
            time.sleep(0.1)
            dc_list = channel.dc_list(voltages=self.qdac_dc_lists[self.qdac_channels.index(channel)],
                                                           dwell_s = 1e-4,
                                                           stepped = True)
            ''' Set the channel trigger input to external'''
            time.sleep(3)
...
class KST_VNA_single_freq_QDac_list(Hard_Sweep):

    def __init__(self, VNA, QDac,
                 freq=None, 
                 if_bandwidth = None,
                 power = None,
                 averages = None, 
        ...
        super(KST_VNA_single_freq_QDac_list, self).__init__()
        self.name = 'KST_VNA_single_freq_QDac_list'
        self.parameter_name = str(qdac_channel_1)
        self.unit = 'V'

        self.VNA = VNA
        self.filename = 'KST_VNA_single_freq_QDac_list'
        ...
        self.QDac = QDac
        print(QDac)
        self.qdac_channel_1 = qdac_channel_1
        self.qdac_ch1_start = qdac_ch1_start
        self.qdac_ch1_stop = qdac_ch1_stop
        self.qdac_ch1_npts = qdac_ch1_npts
    ...
    def prepare(self):
        '''
        Prepare the VNA and QDac Channel for measurements by defining basic settings.
        Set the frequency sweep and get the frequency points back from the insturment
        
        Set all the trigger functions
      ...
      # VNA.start_sweep_all()
      # setup which parameter to measure
       # str_to_write = "CALC:MEAS:PAR '%s'" % self.measure
        #print(str_to_write)
        #self.VNA.visa_handle.write(str_to_write) 
        self.VNA.bandwidth(self.if_bandwidth)
        self.VNA.delay_t(self.delay_t)
        
        print(self.VNA.delay_t(),"delay time")
        # trigger signal is generated with the command:
        # VNA.start_sweep_all()
        self.VNA.rf_on()
        ...
        if self.segment_list == None:
            ...
            self.VNA.npts(self.qdac_ch1_npts*2)
            self.VNA.trigger_output('on')
            self.VNA.trigger_output_duration(2e-5)
            self.VNA.trigger_output_mode('POIN')
            self.VNA.trigger_output_sign('POS')
            self.VNA.trigger_output_pos('AFT')
            self.VNA.sweep_dwell_time(1e-4)#set vna dwell time for debug 
            
            ''' Prepare the QDAc '''
            ''' setup lsit points and trigger mode, and starting trigger mode, and turn on the voltage output, dwell tme, filter type (medium) '''
            #delta_dc = (self.qdac_ch1_stop-self.qdac_ch1_start)/(self.qdac_ch1_npts-1)
            self.qdac_channel_1.output_filter('med')
            time.sleep(0.1)
            self.dc_list = self.qdac_channel_1.dc_list(voltages=np.linspace(self.qdac_ch1_start,
                                                                       self.qdac_ch1_stop,
                                                                       self.qdac_ch1_npts).tolist() + 
                                                           np.linspace(self.qdac_ch1_stop,
                                                                       self.qdac_ch1_start,
                                                                       self.qdac_ch1_npts).tolist(),
                                                                       dwell_s = 1e-4,
                                                                       stepped = True)
            ''' Set the channel trigger input to external'''
            time.sleep(3)
            self.dc_list.start_on_external(1)   
            time.sleep(3)
            #print("dc_list points", self.dc_list.points())
           #print(np.linspace(self.qdac_ch1_start,self.qdac_ch1_stop,self.qdac_ch1_npts).tolist())
            #dc_list.start()                                                                
            #dc_list.start() # n corrosponding to the trigger input
            
          
           
        
            
            ''' Rest the channel 1 DC voltage to near start voltage'''
           #self.qdac_channel_1.dc_constant_V(self.qdac_ch1_start)
            ''' start the trigger'''
        ...
        # get the list of frequency used in the span from the VNA
        self.sweep_points =  np.linspace(self.qdac_ch1_start,
                                         self.qdac_ch1_stop,
                                         self.qdac_ch1_npts)
        #print("sweep point")
        #print(self.sweep_points)
    ...
    def finish(self, **kw):
        self.VNA.rf_off() #turn off the VNA
        self.dc_list.abort()
        #self.qdac_channel_1.dc_constant_V(self.qdac_ch1_stop) # keep the DC channel 1 at the finished state



class KST_VNA_single_freq_Anritsu_list_manual_trigger(Hard_Sweep):

    def __init__(self, VNA, Sgen,
                 freq=None, 
                 if_bandwidth = None,
                 power = None,
                 averages = None, 
        ...
        super(KST_VNA_single_freq_Anritsu_list_manual_trigger, self).__init__()
        self.name = 'KST_VNA_single_freq_Anritsu_list_manual_trigger'
        self.parameter_name = "pump frequency"
        self.unit = 'Hz'

        self.VNA = VNA
        self.filename = 'KST_VNA_single_freq_Anritsu_list_manual_trigger'
        self.start_freq = freq
        self.stop_freq = freq
        self.center_freq = freq
        self.power = power
        ...
        self.pump_f_start = pump_f_start
        self.pump_f_stop = pump_f_stop
        self.pump_f_npts = pump_f_npts
    ...
    def prepare(self):
      ...
      # trigger signal is generated with the command:
      # VNA.start_sweep_all()
      # setup which parameter to measurestart the trigger
       # str_to_write = "CALC:MEAS:PAR '%s'" % self.measure
        #print(str_to_write)
        #self.VNA.visa_handle.write(str_to_write) 
        self.VNA.bandwidth(self.if_bandwidth)
        ...
        if self.segment_list == None:
            ...
            self.VNA.npts(self.pump_f_npts)
            self.VNA.trigger_output('off') # disable AUX trigger 1 on output to prevent false trigger on the QDac
            self.VNA.trigger2_output('on')
            self.VNA.trigger2_output_duration(1e-5)
            self.VNA.trigger2_output_mode('POIN')
            self.VNA.trigger2_output_sign('NEG')
            self.VNA.trigger_output_pos('AFT')
            self.VNA.sweep_dwell_time(10e-3)#set vna dwell time for Anritsu to switch frequencies 
            
            ''' Prepare the Anritsu Signal generator '''
            ''' setup lsit points and trigger mode, and starting trigger mode'''
            #delta_dc = (self.qdac_ch1_stop-self.qdac_ch1_start)/(self.qdac_ch1_npts-1)
            
            
            ## Prepare the frequency and power list###
            self.pump_f_step = (self.pump_f_stop-self.pump_f_start)/(self.pump_f_npts-1)
            self.Sgen.set_list_freqs(self.pump_f_start,self.pump_f_step,self.pump_power,0,self.pump_f_npts)
            self.Sgen.on('ON')  
        ...
        # get the list of frequency used in the span from the VNA
        self.sweep_points =  np.linspace(self.pump_f_start,
                                         self.pump_f_stop,
                                         self.pump_f_npts)
        #print("sweep point")
        #print(self.sweep_points)
    ...
    def finish(self, **kw):
        self.VNA.rf_off() #turn off the VNA
        #self.Sgen.set_to_CW_mode() # # set Arintsu to CW mode
        self.Sgen.on("OFF")# turn off the Arintsu
        #self.qdac_channel_1.dc_constant_V(self.qdac_ch1_stop) # keep the DC channel 1 at the finished state
    
        

class KST_VNA_single_freq_Anritsu_freq_sweep(Hard_Sweep):

    def __init__(self, VNA, Anritsu,
                 freq=None, 
                 if_bandwidth = None,
                 averages = None, 
                 measure ='S21',
        ...
        super(KST_VNA_single_freq_Anritsu_freq_sweep, self).__init__()
        self.name = 'KST_VNA_single_freq_Anritsu_freq_sweep'
        self.parameter_name = 'pump freq'
        self.unit = 'Hz'
        self.sweep_control = 'soft'

        self.VNA = VNA
        self.filename = 'KST_VNA_single_freq_Anritsu_freq_sweep'
        self.start_freq = freq
        self.stop_freq = freq
        self.center_freq = freq
        self.measure= measure
        ...
        self.awg_start =pump_freq_start
        self.awg_stop =  pump_freq_stop
        self.awg_npts = pump_npts

        if force_reset == True:
            VNA.reset()
            
    def set_parameter(self, pump_freq):
        
        self.awg.frequency(pump_freq)
    ...
    def prepare(self):
        ...
        self.sweep_points =  np.linspace(self.awg_start,
                                         self.awg_stop,
                                         self.awg_npts)
        ...
        if self.segment_list == None:
            ...
            #self.VNA.npts(self.qdac_ch1_npts)
            #self.VNA.trigger_output('on')
            #self.VNA.trigger_output_duration(1e-6)
            #self.VNA.trigger_output_mode('POIN')
            #self.VNA.trigger_output_sign('POS')
            #self.VNA.trigger_output_pos('AFT')
            #self.VNA.sweep_dwell_time(1e-1)#set vna dwell time for debug 
            
            ''' Prepare the Anritsu AWG '''
            ''' setup lsit points and trigger mode, and starting trigger mode, and turn on the voltage output, dwell tme, filter type (medium) '''
